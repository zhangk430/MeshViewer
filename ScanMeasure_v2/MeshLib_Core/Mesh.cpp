...
Mesh::Mesh() {boundaryLabeled = false;}
...
{	
	...
	//fid2ind.rehash(fid2ind.size());
	LabelBoundaries();

	//Optional Assign Index to Halfedges
	IndexingHalfedges();
...
{
	for (VertexOutHalfedgeIterator heit(v0); !heit.end(); ++heit)
	{
		Halfedge * he = *heit;
		if (he->target()==v1)
			return he;
...
{
	stdext::hash_map<int, int>::iterator cpos = fid2ind.find(id);
	if (cpos == fid2ind.end())
		return NULL;
	else
	{
		Face * f = m_faces[cpos->second];
		return f;
	}
}
...
{
	stdext::hash_map<int, int>::iterator cpos = vid2ind.find(id);
	return (cpos == vid2ind.end())?(NULL):(m_verts[cpos->second]);
}
...
{		
	...
	for(i = 0; i < 3; i ++ )
	{
		...
		hes[i]->target() = ver[i];
		ver[i]->he() = hes[i];
		std::vector<Halfedge *> & adjInHEList = v_adjInHEList[ver[i]->index()];
